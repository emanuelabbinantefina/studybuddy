...
const FIXED_ITALIAN_HOLIDAYS = new Map([
  ['01-01', 'Capodanno'],
  ['01-06', 'Epifania'],
...
function getItalianOfficialHolidayName(date) {
...
function getItalianExamDateValidationError(raw, options = {}) {
  const { disallowPast = false } = options;
  const examDate = raw instanceof Date ? toStartOfDay(raw) : parseDateOnly(raw);
...
module.exports = {
  getItalianExamDateValidationError,
  getItalianOfficialHolidayName,
  parseDateOnly,
  toStartOfDay,
};
